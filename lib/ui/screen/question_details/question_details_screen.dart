...
class QuestionDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    QuestionDetailsScreenArgs args =
...
class QuestionDetailsWidgetState extends State<QuestionDetailsWidget> {
  ...
  List<QuestionAddBean> newReply = <QuestionAddBean>[];
  late List<ReplyBean> aList;

  @override
  void initState() {
    super.initState();
    _bloc = BlocProvider.of<QuestionDetailsBloc>(context)
      ..add(
        FetchEvent(),
      );
    aList = (widget.questionBean.replies ?? []).getNonNullItems();
  }
    ...
    return BlocListener(
      ...
      listener: (context, state) {
        if (state is ReplyAddedState) {
          setState(() {
            if (state.questionAddResponse.comment != null) {
              newReply.insert(0, state.questionAddResponse.comment!);
            }
            ...
            child: Column(
                ...
                children: <Widget>[
                  ...
                  Padding(
                    ...
                    child: ((widget.questionBean.replies?.length ?? 0) != 0)
                        ? _buildList(aList)
                        : Center(),
                  ),
                ]),
  ...
  addReply() {
    _bloc.add(
      QuestionAddEvent(widget.lessonId, _reply.text,
          int.tryParse(widget.questionBean.comment_ID ?? '') ?? 0),
    );
    _reply.clear();
  }
