    ...
    if (state.userPlans.isNotEmpty && _haveValidPlan(state.userPlans)) {
      state.userPlans.forEach((value) {
        list.add(_buildPriceItem(
            (selectedId == (int.tryParse(value.subscription_id ?? '0') ?? 0)),
            localizations.getLocalization("enroll_with_membership"),
            value.name,
            value.quotas_left, () {
          setState(() {
            selectedId = int.tryParse(value.subscription_id ?? '') ?? 0;
          });
        }));
      });
      ...
      _bloc.availablePlans.forEach((value) {
        list.add(_buildPriceItem(
            (selectedId == (int.tryParse(value.id ?? '') ?? 0)),
            ...
            if (state is LoadedCourseState && state.userPlans.isNotEmpty) {
              _bloc.add(
                PaymentSelectedEvent(
                    selectedId, state.courseDetailResponse.id?.toInt() ?? 0),
              );
              Navigator.pop(
                context,
              );
