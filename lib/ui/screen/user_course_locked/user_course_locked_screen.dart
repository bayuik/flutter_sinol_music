...
class UserCourseLockedScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final UserCourseLockedScreenArgs args = ModalRoute.of(context)
        ?.settings
        .arguments as UserCourseLockedScreenArgs;
    return BlocProvider(
                ...
                if (state is LoadedCourseState) {
                  if (!(state.courseDetailResponse.has_access ?? false)) {
                    if (_bloc.selectedPaymetId == -1) {
                      if (state.courseDetailResponse.id != null) {
                        _bloc.add(
                            AddToCart(state.courseDetailResponse.id!.toInt()));
                      }
                    } else {
                      if (state.courseDetailResponse.id != null) {
                        _bloc.add(
                            UsePlan(state.courseDetailResponse.id!.toInt()));
                      }
                    }
                  }
                }
          ...
          if (_bloc.selectedPaymetId == -1) {
            ...
            if (state.userPlans.isNotEmpty) {
              state.userPlans.forEach((value) {
                if (int.parse(value.id ?? "0") == _bloc.selectedPaymetId)
          ...
          case "video":
            Navigator.of(context).pushReplacementNamed(
              ...
              arguments: LessonVideoScreenArgs(
                  ...
                  state.courseDetailResponse.first_lesson?.toInt() ?? 0,
                  state.courseDetailResponse.author?.avatar_url ?? "",
                  state.courseDetailResponse.author?.login ?? "",
                  true,
                  state.courseDetailResponse.trial ?? false),
            );
            break;
          default:
            Navigator.of(context).pushReplacementNamed(
