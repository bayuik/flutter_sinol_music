...
class DetailProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final DetailProfileScreenArgs args =
        ModalRoute.of(context)?.settings.arguments as DetailProfileScreenArgs;
                                                ...
                                                Padding(
                                                  padding:
                                                      const EdgeInsets.only(
                                                          left: 8.0),
                                                  child: Text(
                                                    "${(_bloc.account?.rating?.average ?? 0).toDouble()} (${_bloc.account?.rating?.total_marks})",
                                                    textScaleFactor: 1.0,
                                                    style: TextStyle(
                                                        fontSize: 16,
                                                        color: Colors.white
    ...
    if (state is LoadedDetailProfileState) {
      if (state.isTeacher) {
          ...
          children: <Widget>[
            ...
            ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                if (item.rating?.total != null) {
                  reviews = item.rating!.total!.toInt();
                }
                return GestureDetector(
                    ...
                    child: Padding(
                      ...
                      child: item.categories_object?.isEmpty ?? true
                          ? Center()
                          : _buildCard(
                              context,
                              item.images?.full,
        ...
        if (_bloc.account?.meta?.description != null &&
            (_bloc.account?.meta?.description?.isNotEmpty ?? false)) {
          return Padding(
            padding: const EdgeInsets.all(32.0),
            child: Text(
              _bloc.account?.meta?.description ?? '',
