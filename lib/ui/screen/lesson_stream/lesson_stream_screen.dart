...
class LessonStreamScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final LessonStreamScreenArgs args =
        ModalRoute.of(context)?.settings.arguments as LessonStreamScreenArgs;
...
class _LessonStreamScreenState extends State<_LessonStreamScreenWidget> {
  ...
  _buildTitle(state) {
    ...
    if (state is LoadedLessonStreamState) {
      return Column(
        ...
        children: <Widget>[
          Text(
            (state.lessonResponse.section?.number).toString() + ":",
            textScaleFactor: 1.0,
  ...
  loadPlayer(state) {
    if (state is LoadedLessonStreamState) {
      String? videoId =
          YoutubePlayer.convertUrlToId(state.lessonResponse.video ?? '');
