...
class UserCourseScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final UserCourseScreenArgs args =
        ModalRoute.of(context)?.settings.arguments as UserCourseScreenArgs;
    return BlocProvider(
...
class UserCourseWidgetState extends State<UserCourseWidget> {
      ...
      case "quiz":
        screenFuture = Navigator.of(context).pushNamed(
          ...
          arguments: QuizLessonScreenArgs(
              int.parse(widget.args.course_id ?? "0"),
              id,
      ...
      case "video":
        screenFuture = Navigator.of(context).pushNamed(
          ...
          arguments: LessonVideoScreenArgs(
            int.tryParse(widget.args.course_id ?? "0") ?? 0,
            id,
            widget.args.avatar_url ?? "",
            widget.args.login ?? "",
            false,
      ...
      case "assignment":
        screenFuture = Navigator.of(context).pushNamed(
          ...
          arguments: AssignmentScreenArgs(
            int.tryParse(widget.args.course_id ?? "") ?? 0,
            id,
            widget.args.avatar_url ?? "",
            widget.args.login ?? "",
          ),
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        if (sectionItem.section_items != null &&
            (sectionItem.section_items?.isNotEmpty ?? false))
          sectionItem.section_items == null
              ? Center()
              : Column(
  ...
  Widget _buildLesson(Section_itemsBean section_itemsBean) {
    bool locked = (section_itemsBean.locked ?? false) && dripContentEnabled;
    Widget icon = Center();
    String duration = section_itemsBean.duration ?? "0";
    ...
    return Padding(
      ...
      child: InkWell(
        onTap: () {
          if (!locked)
            _openLesson(
              section_itemsBean.type ?? "",
              section_itemsBean.item_id ?? 0,
            );
        },
        child: Container(
          decoration: BoxDecoration(
            color: HexColor.fromHex("#F3F5F9"),
          ),
          child: Padding(
            padding: const EdgeInsets.only(
              top: 16.0,
                  ...
                  children: <Widget>[
                    Visibility(
                      ...
                      child: SizedBox(
                        ...
                        height: 24,
                        child: SvgPicture.asset(
                          "assets/icons/ico_lock.svg",
                          color: mainColor,
                        ),
                      ),
                    ),
                    Visibility(
                      visible: !locked && duration.isNotEmpty,
                      child: Row(
                        mainAxisSize: MainAxisSize.min,
