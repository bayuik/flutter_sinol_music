...
class LessonRepositoryImpl extends LessonRepository {
  ...
  Future<List<LessonResponse>> getAllLessons(
    ...
    await Future.forEach<int>(lessonIds, (element) async {
    ...
    //downloading images
    await Future.forEach<LessonResponse>(lessons, (element) async {
      var index = lessons.indexOf(element);

      //Removing srcset tags
      var setsToRemove = lessons[index].content == null
          ? null
          : _getSrcSets(lessons[index].content!);
      if (setsToRemove != null)
        for (String set in setsToRemove) {
          var currentElement = element;
          currentElement.content =
              currentElement.content?.replaceFirst(set, "");
          lessons[lessons.indexOf(element)] = currentElement;
        }

      List<String> urls = (element).content == null
          ? []
          : _getLinkedFileUrls((element).content!);
