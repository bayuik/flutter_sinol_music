...
import '../models/purchase/post.dart';

@singleton
class UserApiProvider {
  // static const BASE_URL = "https://stylemixthemes.com/masterstudy/academy";
  static const BASE_URL = "https://musik.sinol.co.id/wp";
  ...
  Future editProfile(
    ...
    await _dio.post(apiEndpoint + "account/edit_profile/",
        data: map,
        options: Options(
          headers: {"requirestoken": "true"},
        ));
  ...
  Future<Map<String, dynamic>> getLocalization() async {
    var data = await _dio.get(UserApiProvider.apiEndpoint);
  ...
  Future<FinalResponse> getCourseResults(int courseId) async {
    ...
    return FinalResponse.fromMap(response.data);
  }

  Future<List<Post>> getArticle() async {
    Response response = await _dio.get(
